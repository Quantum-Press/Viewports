...
export const findBlockDifferences = (
	...
	for( let index = 0; index < combinedProperties.length; index++ ) {
		...
		const collapsedChangesValue = traverseGet( [ property ], collapsedBlockChanges );
		const collapsedRemovesValue = traverseGet( [ property ], collapsedBlockRemoves );
		const collapsedSavesValue = traverseGet( [ property ], collapsedBlockSaves );
		...
		updateBlockDifferences(
			...
			property,
			stylesValue,
			collapsedChangesValue,
			collapsedRemovesValue,
			collapsedSavesValue
		);
	}
