import type { Size } from '@viewports/hooks';
import { DEFAULT_STATE } from '@viewports/store';
import {
	isInMobileRange,
	isInTabletRange,
	isInDesktopRange,
...
export const getViewportBlockValids = ( state : State, clientId : string ) : object => {
	...
	const blockSaves : ViewportSet = cloneDeep( traverseGet( [ clientId ], saves, {} ) );
	const blockChanges : ViewportSet = cloneDeep( traverseGet( [ clientId ], changes, {} ) );
	const blockRemoves : ViewportSet = cloneDeep( traverseGet( [ clientId ], removes, {} ) );
	...
	return findViewportStyle( cloneDeep( blockValids[ last ] ) );
};
