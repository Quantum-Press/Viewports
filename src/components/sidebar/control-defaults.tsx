...
/**
 * Set component const to export sidebar control-defaults ui.
 *
 * @param object props
 *
 * @since 0.1.0
...
const ControlDefaults = () => {
	...
	// Setup has indicator.
	const hasBlockDefaults = select( STORE_NAME ).hasBlockDefaults( clientId );
	...
	const blockDefaults = select( STORE_NAME ).getBlockDefaults( clientId );
	...
	const onClickControl = () => {
		setOpen( hasBlockDefaults && ! open ? true : false );
	}
	...
	const onClickRemove = ( keys : Array<any> ) => {
		...
		dispatch( STORE_NAME ).removeBlockDefaults( clientId, props, viewport );
		setUpdate( true );
	}

	// Setup classNames.
	let classNames = 'qp-viewports-sidebar-control defaults';
	if( hasBlockDefaults ) {
					...
					{ 'object' === view &&
						<div className="qp-viewports-style-object-tree">
							<code className="object-tree">
								{ Object.entries( blockDefaults ).map( ( style, index ) => {
									const origKeys = [];
