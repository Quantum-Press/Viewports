...
class CSSRuleset {
	...
	public function __construct(
		...
		$this->blockHTML = null !== $blockHTML ? $blockHTML : ( isset( $block[ 'innerHTML' ] ) ? $block[ 'innerHTML' ] : '' );
	...
	protected function processSelector( $html, $selectorParts, $callback = null ) : array
	{
		...
		$processNestedSelector = function( $html, $selectorParts, $callback ) use ( &$foundElements, &$processNestedSelector ) {
		...
		foreach( $this->inlineRules as &$inlineRule ) {
			foreach( $inlineRule->getCSSProperties() as $property => $value ) {
				...
				foreach( $this->attributeRules as $attributeRule ) {
					if( 0 !== $attributeRule->getMinWidth() || 'screen' !== $attributeRule->getMedia() ) {
						continue;
					}
